...
size_t Descriptor::getDescriptorSize() const
{
...
double Descriptor::getWinSigma() const
{
    return winSigma >= 0 ? winSigma : (blockSize.width + blockSize.height)/8.;
}

bool Descriptor::checkDetectorSize() const
{
...
void Descriptor::setSVMDetector(const vector<float>& _svmDetector)
{
    svmDetector = _svmDetector;
    CV_Assert( checkDetectorSize() );
}

bool Descriptor::load(const String& filename, const String& objname)
{
...
void Descriptor::save(const String& filename, const String& objName) const
{
...
void Descriptor::computeGradient(const Mat& img, Mat& grad, Mat& qangle,
...
struct HOGCache
{
    ...
    HOGCache();
    HOGCache(const Descriptor* descriptor,
        const Mat& img, Size paddingTL, Size paddingBR,
        bool useCache, Size cacheStride);
    virtual ~HOGCache() {};
    virtual void init(const Descriptor* descriptor,
        const Mat& img, Size paddingTL, Size paddingBR,
        bool useCache, Size cacheStride);
...
HOGCache::HOGCache(const Descriptor* _descriptor,
...
void HOGCache::init(const Descriptor* _descriptor,
        const Mat& _img, Size _paddingTL, Size _paddingBR,
        bool _useCache, Size _cacheStride)
{
    ...
    //   1. loop over windows is unrolled in the Descriptor::{compute|detect} methods;
...
void Descriptor::compute(const Mat& img, vector<float>& descriptors,
                            ...
                            const vector<Point>& locations) const
{
    ...
    padding.width = (int)alignSize(max(padding.width, 0), cacheStride.width);
    padding.height = (int)alignSize(max(padding.height, 0), cacheStride.height);
...
void Descriptor::detect(const Mat& img,
...
void Descriptor::detectMultiScale(
...
vector<float> Descriptor::getDefaultPeopleDetector()
{
